...
import xmltodict, json, pprint, os
...
def cli_stats(task, **kwargs):
    ...
    if task.host.platform == "alcatel_sros":
        vprn = get_vprncli(task, servicename)
        # Create the path folder directory.
        createFolder(path)
        # Capture the get_vprn output and write it to a file:
        write_file(task, filename=f"{path}/{task.name}-{servicename}.txt", content=vprn)

    elif task.host.platform == "iosxr":
        vrf = get_vrfcli(task, servicename)
        # Create the path folder directory.
        createFolder(path)
        # Capture the get_vprn output and write it to a file:
        write_file(task, filename=f"{path}/{task.name}-{servicename}.txt", content=vrf)
